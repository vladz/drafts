...
class AtolOnline:
    ...
    async def get_token(self):
        async with ClientSession() as session:
            resp = await session.post('https://online.atol.ru/possystem/v3/getToken',
    ...
    async def post_sell(self, invoice, date, email, phone, items, amount):
        async with ClientSession() as session:
            data = {'external_id': invoice,
                    ...
                    'receipt': {
                        'attributes': {'sno': 'osn', 'email': email, 'phone': f'+{phone}'},
                        'items': items,
                        'payments': [{'type': 1, 'sum': amount}],
                        'total': amount}}
            resp = await session.post(f'https://online.atol.ru/possystem/v3/{ATOL_GROUP}/sell?tokenid={self.token}',
                                      json=data)
            resp_msg = await resp.text()
            ...
            if resp.status == 200:
                ...
                await self.post_sell(invoice, date, email, phone, items, amount)
            else:
